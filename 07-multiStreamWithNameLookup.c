/***************************************************************************************************
* Created by JEP on Nov 13, 2016 testing of Linux poll() and related code.   KEY Features:         *
* 1. name to IP address lookup, no need for hard coded IP address.                                 *
* 2. Open and correct ussage (on BeagleBone at least) of GPIO bits so poll() will work             *
* 3. Keyboard input (just basic line input however, no key by key support)                         *
* 4. Network send of events, key board input                                                       *
* 5. Network receive of whatever we want                                                           *
*                                                                                                  *
* To compile:                                                                                      *
* gcc -Wall  -oprog07  07-multiStreamWithNameLookup.c                                              *
*                                                                                                  *
* NOTE:                                                                                            *
...
#include <sys/time.h>
#include <time.h>        // time(), time_t, strftime()

extern int errno;        // required for sterror() usage

#include <sys/ioctl.h>
#include <sys/poll.h>
#include <netdb.h>       // getaddrinfo(), inet_ntop(), related stuff
...
char serverName[500];
char serverIP_Addr[100];
int  serverPORT_Num = 3033;
...
void setupSocketData (char *name, int entry, PROCESS_INPUT func) {
    ...
    printf ("%03d: Connecting to %s, on port %d \n", __LINE__, serverIP_Addr, serverPORT_Num);
    ...
    myFD_Ctrl[entry].si_other.sin_port = htons(serverPORT_Num);
     
    if (inet_aton(serverIP_Addr , &myFD_Ctrl[entry].si_other.sin_addr) == 0) {
...
int main (int argc, char *argv[])
{
    ...
    strcpy (serverName, "45.55.156.55");
    strcpy (serverName, "www.jep.cc");

    lookup_host (serverIP_Addr, sizeof serverIP_Addr, serverName);
    ...
    {
        char msgBuf[200], msgTimeStr[200];
        time_t now_t = time(0);
        struct tm * now_tm = localtime( &now_t );

//      sprintf (msgBuf, "%s is now running.  %04d-%02d-%02d %02d:%02d:%02d", now_tm.year + 1900, now_tm->tm_mon + 1, now_tm->tm_mday, now_tm->hour, now_tm->min, 

        strftime (msgTimeStr, sizeof msgTimeStr - 1, "%Y-%b-%d %X %z", now_tm);
        sprintf (msgBuf, "%s is now running.  %s ", __FILE__, msgTimeStr);
        sendMessageToSocket (msgBuf, entrySTDIN);   // Tell server we are awake, who we are
    }
